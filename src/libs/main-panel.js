...
export class MainPanelProvider {
  ...
  loadStyle(styles) {
    if (styles.length > 0) {
      const styleNodeOld = document.querySelectorAll('[data-id="newScoped"]')[0]
      if (styleNodeOld) {
        document.head.removeChild(styleNodeOld)
      }
      ...
      styleNode.setAttribute('data-id', 'newScoped')
