...
function compileJsCode(code, onEncounterDuplicateDeclared = () => {}) {
  ...
  // 提取data中返回的对象结构, 遍历拆分不同js类型
  const body = ast.body

  body.forEach(item => {
    //ImportDeclaration  为import    ExportDefaultDeclaration为js export
    if (item.type == 'ImportDeclaration') {
      // item.specifiers 为list  item.type为ImportSpecifier 表示{}引入
      //                         item.type为ImportDefaultSpecifier 表示 直接引入
      // console.log(item.specifiers);
    }

    if (item.type == 'ExportDefaultDeclaration') {
      const properties = item.declaration.properties

      //暂时只对data，methods取值
      properties.length > 0 &&
        properties.forEach(item => {
          if (item.key.name == 'data') {
            item.value.body.body.forEach(item => {
              if (item.type == 'ReturnStatement') {
                const newCode = escodegen.generate(item.argument)
          ...
          if (item.key.name == 'methods') {
            const methodsAst = item.value.properties
            methodsAst &&
